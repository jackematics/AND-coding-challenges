    ...
    describe('when the ant is about to cross a grid boundary', () => {
      ...
      describe('bottom boundary', () => {
        ...
        it('should adjust the ant grid index accordingly', async () => {
          ...
          await waitFor(() => {
            expect(result.current.antGridData.antData.gridIndex.row).toBe(3);
            expect(result.current.antGridData.antData.gridIndex.col).toBe(1);
          });
        });
      });
      ...
      describe('left boundary', () => {
        it('should add an extra column of white cells at the left boundary', async () => {
          const { result } = renderHook(() =>
            useAntModel(
              new AntGridModeller(
                new Ant(
          ...
          await waitFor(() => {
            expect(result.current.antGridData.gridData).toStrictEqual(
              AntModelTestData.gridLeftBoundaryAdded
            );
          });
        });
